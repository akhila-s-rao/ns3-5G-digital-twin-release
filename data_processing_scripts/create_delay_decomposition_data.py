...
def build_lena_delay_decomposition_table(
    ...
    table = pd.DataFrame(columns=PACKET_KEYS)

    if df_ul_pdcp_rx is not None:
        pdcp_rx = df_ul_pdcp_rx[["rnti", "lcid", "pkt_id", "packet_size", "time_us", "delay_us"]].copy()
        for col in ["rnti", "lcid", "pkt_id", "packet_size", "time_us", "delay_us"]:
            pdcp_rx[col] = pd.to_numeric(pdcp_rx[col], errors="coerce")
        pdcp_rx = pdcp_rx.dropna(subset=["rnti", "lcid", "pkt_id", "time_us", "delay_us"])
        if not pdcp_rx.empty:
            ...
            table = (
                pdcp_rx[["rnti", "lcid", "pkt_id", "pkt_size_bytes", "pdcp_rx_time_us", "ran_delay_ms"]]
...
def load_lena_delay_decomposition(input_dir: Path) -> pd.DataFrame | None:
    ...
    try:
        df_ul_rlc = require_optional_columns(
            df_ul_rlc, "NrUlRlcRxComponentStats.txt", {"time_us", "rnti", "lcid", "pkt_id", "delay_us"}
        )
        df_ul_rlc_tx = require_optional_columns(
            df_ul_rlc_tx, "NrUlRlcTxComponentStats.txt", {"time_us", "rnti", "lcid", "pkt_id", "rlc_sn"}
        )
        df_ul_pdcp_tx = require_optional_columns(
            df_ul_pdcp_tx, "NrUlPdcpTxStats.txt", {"time_us", "rnti", "lcid", "pkt_id"}
        )
        df_ul_pdcp_rx = require_optional_columns(
            df_ul_pdcp_rx, "NrUlPdcpRxStats.txt", {"time_us", "rnti", "lcid", "pkt_id", "packet_size", "delay_us"}
        )
        df_rlc_sojourn = require_optional_columns(
            df_rlc_sojourn, "RlcTxQueueSojournTrace.txt", {"time_us", "rnti", "lcid", "pkt_id", "pre_hol_wait_us"}
        )
        df_rlc_hol_wait = require_optional_columns(
            df_rlc_hol_wait, "RlcHolGrantWaitTrace.txt", {"time_us", "rnti", "lcid", "pkt_id", "hol_grant_wait_us"}
        )
    ...
    if df_ul_rlc is not None:
        ...
        if not df_ul_rlc_non_null.empty:
            idx = df_ul_rlc_non_null.groupby(PACKET_KEYS)["delay_ms"].idxmax()
    ...
    if df_rlc_hol_wait is not None and df_ul_rlc is not None:
        grant = df_rlc_hol_wait[["rnti", "lcid", "pkt_id", "time_us"]].copy()
        ...
        grant = grant.dropna(subset=["rnti", "lcid", "pkt_id", "time_us"])
        ...
        df_link_delay = pd.merge(grant, rlc_rx, on=PACKET_KEYS, how="inner")
    ...
    if df_link_delay is not None and df_ul_rlc_plot is not None:
        link = df_link_delay[["rnti", "lcid", "pkt_id", "link_delay_ms", "time_us"]].rename(
            columns={"time_us": "link_time_us"}
        )
        txretx = df_ul_rlc_plot[["rnti", "lcid", "pkt_id", "delay_ms", "time_us"]].rename(
    ...
    if df_ul_rlc_tx is not None:
        tx_comp = df_ul_rlc_tx[["time_us", "rnti", "lcid", "pkt_id", "rlc_sn"]].copy()
        for col in ["time_us", "rnti", "lcid", "pkt_id", "rlc_sn"]:
            tx_comp[col] = pd.to_numeric(tx_comp[col], errors="coerce")
        tx_comp = tx_comp.dropna(subset=["time_us", "rnti", "lcid", "pkt_id", "rlc_sn"])
        if not tx_comp.empty:
            tx_comp = tx_comp.sort_values("time_us")
            first_tx = (
                tx_comp.groupby(PACKET_KEYS, as_index=False)["time_us"]
                .min()
                .rename(columns={"time_us": "first_tx_time_us"})
            )
            seg_count = (
                tx_comp.groupby(PACKET_KEYS, as_index=False)["rlc_sn"]
                .nunique()
                .rename(columns={"rlc_sn": "rlc_segments_per_pkt"})
            )
            df_rlc_segments_per_pkt = pd.merge(first_tx, seg_count, on=PACKET_KEYS, how="inner")
    ...
    if df_sr is not None and df_rlc_segments_per_pkt is not None:
        ...
        first_pkt_tx = df_rlc_segments_per_pkt[["time_us", "rnti", "lcid", "pkt_id"]].copy()
        first_pkt_tx = first_pkt_tx.rename(columns={"time_us": "first_pkt_tx_time_us"})
        for col in ["first_pkt_tx_time_us", "rnti", "lcid", "pkt_id"]:
            first_pkt_tx[col] = pd.to_numeric(first_pkt_tx[col], errors="coerce")
        first_pkt_tx = first_pkt_tx.dropna(subset=["first_pkt_tx_time_us", "rnti", "lcid", "pkt_id"])
        ...
        if not first_pkt_tx.empty:
            first_pkt_tx["prev_first_pkt_tx_time_us"] = (
                first_pkt_tx.groupby(["rnti", "lcid"])["first_pkt_tx_time_us"].shift(1)
            )
    ...
    if df_sr is not None and df_ul_pdcp_tx is not None:
        sr = df_sr.copy()
        pdcp_tx_cols = ["time_us", "rnti", "lcid", "pkt_id"]
        ...
        for col in ["pdcp_tx_time_us", "rnti", "lcid", "pkt_id"]:
            pdcp_tx[col] = pd.to_numeric(pdcp_tx[col], errors="coerce")
        pdcp_tx = pdcp_tx.dropna(subset=["pdcp_tx_time_us", "rnti", "lcid", "pkt_id"]).sort_values(["pdcp_tx_time_us", "rnti"])
...
def write_lena_delay_decomposition_csv(lena_run_dir: Path, csv_output_dir: Path) -> None:
...
def main():
    ...
    for lena_run_dir in sorted(lena_runs):
        write_lena_delay_decomposition_csv(lena_run_dir, csv_output_dir)
