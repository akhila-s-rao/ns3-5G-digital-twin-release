...
class NrSpectrumPhy : public SpectrumPhy
{
  public:
    ...
    typedef void (*RxDataTracedCallback)(const SfnSf& sfnSf,
                                         ...
                                         uint16_t bwpId,
                                         uint16_t cellId);
    // codex added: trace callback for MAC TB delay (first MAC TX -> successful RX)
    typedef void (*MacTbDelayTracedCallback)(uint16_t cellId,
                                             uint16_t rnti,
                                             uint8_t bwpId,
                                             bool isDownlink,
                                             uint64_t delayNs,
                                             uint32_t ipId);
    // codex added: trace callback for component-level PHY RX packet rows.
    typedef void (*RxPacketTraceComponentTracedCallback)(RxPacketTraceParams params,
                                                          uint8_t lcid,
                                                          uint64_t rxPduId,
                                                          uint32_t rxPduBytes,
                                                          uint32_t pktId,
  ...
  private:
    ...
    TracedCallback<RxPacketTraceParams, uint8_t, uint64_t, uint32_t, uint32_t, uint32_t>
        m_rxPacketTraceGnbComponents; //!< codex added: component-level rows for gNB PHY RX packets
    TracedCallback<RxPacketTraceParams, uint8_t, uint64_t, uint32_t, uint32_t, uint32_t>
        m_rxPacketTraceUeComponents; //!< codex added: component-level rows for UE PHY RX packets
    TracedCallback<uint16_t, uint16_t, uint8_t, bool, uint64_t, uint32_t>
        m_macTbDelayTrace; //!< codex added: MAC TB delay trace (first MAC TX -> successful RX)
    ...
    uint64_t m_rxPduTraceId = 0; // codex added: monotonically increasing PHY RX PDU id per PHY
    bool m_isGnb = false;
