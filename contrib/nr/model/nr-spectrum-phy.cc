...
#include "nr-mac-tx-time-tag.h" // codex added
    ...
    static TypeId tid =
        TypeId("ns3::NrSpectrumPhy")
            ...
            .AddTraceSource("RxPacketTraceUe",
                            ...
                            "ns3::RxPacketTraceParams::TracedCallback")
            .AddTraceSource("RxPacketTraceGnbComponents",
                            "Component-level rows for packets received at gNB PHY",
                            MakeTraceSourceAccessor(&NrSpectrumPhy::m_rxPacketTraceGnbComponents),
                            "ns3::NrSpectrumPhy::RxPacketTraceComponentTracedCallback")
            .AddTraceSource("RxPacketTraceUeComponents",
                            "Component-level rows for packets received at UE PHY",
                            MakeTraceSourceAccessor(&NrSpectrumPhy::m_rxPacketTraceUeComponents),
                            "ns3::NrSpectrumPhy::RxPacketTraceComponentTracedCallback")
            .AddTraceSource("MacTbDelay",
                            "MAC TB delay (first MAC TX -> successful RX) with HARQ retries",
                            MakeTraceSourceAccessor(&NrSpectrumPhy::m_macTbDelayTrace),
                            "ns3::NrSpectrumPhy::MacTbDelayTracedCallback") // codex added
            .AddTraceSource(
                "ChannelOccupied",
...
NrSpectrumPhy::ProcessReceivedPacketBurst()
{
    ...
    for (auto packetBurst : m_rxPacketBurstList)
    {
        for (auto packet : packetBurst->GetPackets())
        {
            ...
            const uint8_t lcid = bearerTag.GetLcid();
            const uint32_t rxPduBytes = packet->GetSize();
            const uint64_t rxPduId = m_rxPduTraceId++;
            ...
            if (!tbInfo.m_isCorrupted)
            {
                // codex added: compute MAC TB delay using first MAC->PHY TX tag.
                NrMacTxTimeTag macTxTag;
                if (packet->PeekPacketTag(macTxTag))
                {
                    const uint16_t cellId =
                        gnbRx ? gnbRx->GetCellId() : ueRx->GetTargetGnb()->GetCellId();
                    const bool isDownlink = tbInfo.m_expected.m_isDownlink;
                    const uint64_t delayNs =
                        (Simulator::Now() - macTxTag.GetTxTime()).GetNanoSeconds();
                    ...
                    m_macTbDelayTrace(cellId, rnti, GetBwpId(), isDownlink, delayNs, ipId);
                }
            ...
            if (gnbRx)
            {
                ...
                for (ByteTagIterator it = packet->GetByteTagIterator(); it.HasNext();)
                {
                    ...
                    m_rxPacketTraceGnbComponents(traceParams,
                                                 lcid,
                                                 rxPduId,
                                                 rxPduBytes,
                                                 ipIdTag.GetId(),
            ...
            else if (ueRx)
            {
                ...
                for (ByteTagIterator it = packet->GetByteTagIterator(); it.HasNext();)
                {
                    ...
                    const uint32_t componentBytes =
                        (item.GetEnd() >= item.GetStart()) ? (item.GetEnd() - item.GetStart()) : 0;
                    m_rxPacketTraceUeComponents(traceParams,
                                                lcid,
                                                rxPduId,
                                                rxPduBytes,
                                                ipIdTag.GetId(),
