...
NrRlcAm::DoTransmitPdcpPdu(Ptr<Packet> p)
{
    ...
    if (m_txonBufferSize + p->GetSize() <= m_maxTxBufferSize || (m_maxTxBufferSize == 0))
    {
        ...
        const bool wasEmpty = m_txonBuffer.empty();
        const Time enqueueTime = Simulator::Now();
        const Time holSince = wasEmpty ? enqueueTime : Time(0);
        m_txonBuffer.emplace_back(p, enqueueTime, holSince);
        if (wasEmpty)
        {
            // codex added: log pre-HOL wait (enqueue -> HOL) when buffer was empty.
            // codex added: include IPv4 identification when present.
            Ipv4IdTag ipIdTag;
            const uint32_t ipId = p->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
            m_txQueueSojourn(m_rnti,
                             m_lcid,
                             p->GetSize(),
                             (holSince - enqueueTime).GetNanoSeconds(),
                             ipId);
        }
...
NrRlcAm::DoNotifyTxOpportunity(NrMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    auto startPeriodicBsrIfNeeded = [this]() {
    ...
    if (m_statusPduRequested && !m_statusProhibitTimer.IsPending())
    {
        ...
        Ipv4IdTag ipIdTag;
        const uint32_t ipId = packet->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
        m_txPdu(m_rnti, m_lcid, packet->GetSize(), ipId);
        TraceTxPduComponents(packet, 0);
        ...
        for (sn = m_vtA; sn < m_vtS; sn++)
        {
            ...
            if (m_retxBuffer.at(seqNumberValue).m_pdu)
            {
                    ...
                    m_txOpportunityForRetxAlwaysBigEnough)
                {
                    ...
                    const uint32_t ipId = packet->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
                    m_txPdu(m_rnti, m_lcid, packet->GetSize(), ipId);
                    TraceTxPduComponents(packet, rlcAmHeader.GetSequenceNumber().GetValue());
    ...
    // codex added: log per-PDU pre-HOL wait time when it becomes HOL.
    auto logPreHolWait = [this](const Ptr<Packet>& pdu, Time holSince, Time waitingSince) {
        if (!pdu)
        {
            return;
        }
        // codex added: include IPv4 identification when present.
        Ipv4IdTag ipIdTag;
        const uint32_t ipId = pdu->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
        m_txQueueSojourn(m_rnti,
                         m_lcid,
                         pdu->GetSize(),
                         (holSince - waitingSince).GetNanoSeconds(),
                         ipId);
    };
    // codex added: log per-PDU/segment HOL-to-grant wait time when it is dequeued for TX.
    auto logHolGrantWait = [this](const Ptr<Packet>& segment, Time holSince) {
        if (!segment)
        {
            return;
        }
        // codex added: include IPv4 identification when present.
        Ipv4IdTag ipIdTag;
        const uint32_t ipId = segment->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
        m_txHolGrantWait(m_rnti,
                         m_lcid,
                         segment->GetSize(),
                         (Simulator::Now() - holSince).GetNanoSeconds(),
                         ipId);
    };
    ...
    Time firstSegmentHol = m_txonBuffer.begin()->m_holSince;
    ...
    if (!m_txonBuffer.empty())
    {
        // codex added: track when the next PDU becomes HOL after dequeue.
        m_txonBuffer.front().m_holSince = Simulator::Now();
        logPreHolWait(m_txonBuffer.front().m_pdu,
                      m_txonBuffer.front().m_holSince,
    ...
    while (firstSegment && (firstSegment->GetSize() > 0) && (nextSegmentSize > 0))
    {
            ...
            (firstSegment->GetSize() > 2047))
        {
            ...
            if (firstSegment->GetSize() > 0)
            {
                ...
                m_txonBuffer.insert(m_txonBuffer.begin(),
                                    TxPdu(firstSegment, firstSegmentTime, firstSegmentHol));
                m_txonBufferSize += m_txonBuffer.begin()->m_pdu->GetSize();
            ...
            logHolGrantWait(newSegment, firstSegmentHol); // codex added
        ...
        else if ((nextSegmentSize - firstSegment->GetSize() <= 2) || m_txonBuffer.empty())
        {
            ...
            logHolGrantWait(firstSegment, firstSegmentHol); // codex added
        ...
        else // (firstSegment->GetSize () < m_nextSegmentSize) && (m_txBuffer.size () > 0)
        {
            ...
            firstSegmentHol = m_txonBuffer.begin()->m_holSince;
            m_txonBufferSize -= m_txonBuffer.begin()->m_pdu->GetSize();
            m_txonBuffer.erase(m_txonBuffer.begin());
            if (!m_txonBuffer.empty())
            {
                // codex added: update HOL time for the new front after dequeue.
                m_txonBuffer.front().m_holSince = Simulator::Now();
                logPreHolWait(m_txonBuffer.front().m_pdu,
                              m_txonBuffer.front().m_holSince,
                              m_txonBuffer.front().m_waitingSince);
...
NrRlcAm::DoReceivePdu(NrMacSapUser::ReceivePduParameters rxPduParams)
{
    ...
    const uint16_t rlcSn = rlcAmHeader.IsDataPdu() ? rlcAmHeader.GetSequenceNumber().GetValue() : 0;
    TraceRxPduComponents(rxPduParams.p, rlcSn, delay.GetNanoSeconds());
