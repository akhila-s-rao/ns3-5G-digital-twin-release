...
#include "ipv4-id-tag.h" // codex added
                     ...
                     Ptr<Ipv4Route> route)
{
    ...
    // codex added: tag packet bytes with a 32-bit tracing ID for cross-layer tracing.
    // Keep the existing id on recursive Send() calls to avoid duplicate tags.
    Ipv4IdTag ipv4IdTag;
    if (!packet->FindFirstMatchingByteTag(ipv4IdTag))
    {
        ipv4IdTag.SetId(m_traceIdentification++);
        packet->AddByteTag(ipv4IdTag);
        pktCopyWithTags->AddByteTag(ipv4IdTag);
    }
