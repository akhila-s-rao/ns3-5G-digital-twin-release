...
NrGnbMac::DoTransmitPdu(NrMacSapProvider::TransmitPduParameters params)
{
    ...
    // codex added: stamp first MAC->PHY TX time for TB delay logging (preserve on retx).
    NrMacTxTimeTag macTxTag;
    if (!params.pdu->PeekPacketTag(macTxTag))
    {
        macTxTag.SetTxTime(Simulator::Now());
        params.pdu->AddPacketTag(macTxTag);
    }
