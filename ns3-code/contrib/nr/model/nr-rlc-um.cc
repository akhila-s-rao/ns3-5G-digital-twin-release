...
NrRlcUm::DoNotifyTxOpportunity(NrMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    Time firstSegmentHol = m_txBuffer.begin()->m_holSince;
    ...
    if (!m_txBuffer.empty())
    {
        ...
        m_txBuffer.front().m_holSince = Simulator::Now();
        logPreHolWait(m_txBuffer.front().m_pdu,
                      m_txBuffer.front().m_holSince,
                      m_txBuffer.front().m_waitingSince);
    }
    ...
    while (firstSegment && (firstSegment->GetSize() > 0) && (nextSegmentSize > 0))
    {
        ...
        else // (firstSegment->GetSize () < m_nextSegmentSize) && (m_txBuffer.size () > 0)
        {
            ...
            firstSegmentHol = m_txBuffer.begin()->m_holSince;
            m_txBufferSize -= firstSegment->GetSize();
    ...
    TraceTxPduComponents(packet, rlcHeader.GetSequenceNumber().GetValue());
...
NrRlcUm::DoReceivePdu(NrMacSapUser::ReceivePduParameters rxPduParams)
{
    ...
    TraceRxPduComponents(rxPduParams.p, seqNumber.GetValue(), delay.GetNanoSeconds());
