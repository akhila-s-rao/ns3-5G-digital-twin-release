...
NrPdcp::DoTransmitPdcpSdu(NrPdcpSapProvider::TransmitPdcpSduParameters params)
{
    ...
    // codex added: propagate IPv4 identification in PDCP traces when present.
    Ipv4IdTag ipIdTag;
    const uint32_t ipId = p->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
    m_txPdu(m_rnti, m_lcid, p->GetSize(), ipId);
...
NrPdcp::DoReceivePdu(Ptr<Packet> p)
{
    ...
    Ipv4IdTag ipIdTag;
    const uint32_t ipId = p->FindFirstMatchingByteTag(ipIdTag) ? ipIdTag.GetId() : 0;
    m_rxPdu(m_rnti, m_lcid, p->GetSize(), delay.GetNanoSeconds(), ipId);
