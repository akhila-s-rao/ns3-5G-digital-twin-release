    ...
    SlotAllocInfo* slotAlloc) const
{
    ...
    for (const auto beamId : GetBeamOrderRR(activeDlHarq))
    {
        ...
        for (auto it : beam.second)
        {
            ...
            else
            {
                ...
                // codex bug fix: avoid double-counting symbols; beam-level decrement handles OFDMA span
            ...
            // codex test: log HARQ DL allocation details for symbol accounting
            NS_LOG_INFO("HARQ DL alloc rnti=" << dciInfoReTx->m_rnti
                                             << " harqId=" << +dciInfoReTx->m_harqProcess
                                             << " symStart=" << +dciInfoReTx->m_symStart
                                             << " numSym=" << +dciInfoReTx->m_numSym
                                             << " currStartSym=" << +currStartingSymbol
                                             << " symAvail=" << +symAvail);
        ...
        if (symAvail > 0)
        {
            ...
            if (beamStartingSymbol == currStartingSymbol)
            {
                ...
                // codex test: track symbol usage vs availability before decrement
                NS_LOG_INFO("HARQ DL symbolsUsedForBeam=" << +symbolsUsedForBeam
                                                        << " symAvail(before)=" << +symAvail
                                                        << " currStartingSymbol=" << +currStartingSymbol);
                currStartingSymbol += symbolsUsedForBeam;
                symAvail -= symbolsUsedForBeam;
                // codex test: track remaining symbols after decrement
                NS_LOG_INFO("HARQ DL symAvail(after)=" << +symAvail
                                                       << " currStartingSymbol=" << +currStartingSymbol);
            }
        }
    }
    ...
    auto usedSymbols = nr::CountUsedSymbolsFromVarAllocTtiRange(
        ...
        slotAlloc->m_varTtiAllocInfo.end());
    // codex test: summarize HARQ symbol usage for this scheduling pass
    NS_LOG_INFO("HARQ DL usedSymbols=" << +usedSymbols
                                       << " startSym=" << +startingPoint->m_sym
                                       << " symAvailEnd=" << +symAvail
                                       << " newDciCount="
                                       << (slotAlloc->m_varTtiAllocInfo.size() - preexistingDciNum));
    return usedSymbols;
}
